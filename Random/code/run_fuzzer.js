...
async function run_fuzzer(config){
    ...
    var constraints_folder="./cons_test";
    ...
    if (config["constr"]==="yes")
        ...
        subdir+="_noconstr";
    var WORKDIR="workdir_random/"+test_package+"/"+subdir;
    var ok_seed=0;
    ...
    await fs.appendFileSync("ok_seed_num.csv","total,"+JSON.stringify(ok_seed)+"\n")

}
// child_process.execFile("run_fuzzer.sh", ["tensorflowjs","./api_config","./config/ci.config"],null,function (err, stdout, stderr) {
